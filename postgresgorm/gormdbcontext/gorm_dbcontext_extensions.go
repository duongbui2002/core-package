...
import (
	...
	"github.com/duongbui2002/core-package/mapper"
	"github.com/duongbui2002/core-package/postgresgorm/contracts"
	"github.com/duongbui2002/core-package/postgresgorm/scopes"
	typeMapper "github.com/duongbui2002/core-package/reflection/typemapper"
	"github.com/iancoleman/strcase"
	uuid "github.com/satori/go.uuid"
)

func Exists[TDataModel interface{}](ctx context.Context, dbContext contracts.GormDBContext, id uuid.UUID) bool {
	var count int64
...
func FindModelByID[TDataModel interface{}, TModel interface{}](
	...
	if result.Error != nil {
		return *new(TModel), customErrors2.NewNotFoundErrorWrap(result.Error, fmt.Sprintf(
	...
	if err != nil {
		return *new(TModel), customErrors2.NewInternalServerErrorWrap(
			err,
...
func FindDataModelByID[TDataModel interface{}](
	...
	result := dbContext.DB().WithContext(ctx).First(&dataModel, id)
	if result.Error != nil {
		return *new(TDataModel), customErrors2.NewNotFoundErrorWrap(
			result.Error,
			fmt.Sprintf(
				"%s with id `%s` not found in the database",
				dataModelName,
...
func DeleteDataModelByID[TDataModel interface{}](
	...
	if !exists {
		return customErrors2.NewNotFoundError(fmt.Sprintf("%s with id `%s` not found in the database",
			dataModelName,
			id.String(),
		))
	}
	...
	if result.Error != nil {
		return customErrors2.NewInternalServerErrorWrap(
			result.Error,
			fmt.Sprintf(
...
func AddModel[TDataModel interface{}, TModel interface{}](
	...
	if result.Error != nil {
		return *new(TModel), customErrors2.NewConflictErrorWrap(
			result.Error,
...
func AddDataModel[TDataModel interface{}](
	...
	result := txDBContext.DB().WithContext(ctx).Create(dataModel)
	if result.Error != nil {
		return *new(TDataModel), customErrors2.NewConflictErrorWrap(
			result.Error,
...
func UpdateDataModel[TDataModel interface{}](
	...
	result := txDBContext.DB().WithContext(ctx).Updates(dataModel)
	if result.Error != nil {
		return *new(TDataModel), customErrors2.NewInternalServerErrorWrap(
			result.Error,
